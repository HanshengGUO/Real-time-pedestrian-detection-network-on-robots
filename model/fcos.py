...
from .backbone.slimdarknet19 import SlimDarknet19
...
class FCOS(nn.Module):
    ...
    def __init__(self,config=None):
        ...
        elif config.backbone == 'slimdarknet19':
            self.backbone = SlimDarknet19(pretrained=config.pretrained)
        self.fpn=FPN(config.fpn_out_channels,use_p5=config.use_p5,backbone=config.backbone)
    ...
    def train(self,mode=True):
        ...
        if self.config.freeze_stage_1:
            if self.config.backbone == 'slimdarknet19':
                print("INFO===>can't freeze backbone stage1 when finetune pruned network")
            else:
